...
//    https://stackoverflow.com/a/71850432
interface CliArgs {
  input: string;      // -i / --input : path to the M365 app zip
  out: string;        // -o / --out   : output directory
  provision: boolean; // --provision  : create Dataverse bot
}
...
(async () => {
  ...
  if (argv.provision) {
    const name =
      agent?.name || app?.name?.short || "Converted Copilot";
    const description =
